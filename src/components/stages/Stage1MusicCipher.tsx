...
export const Stage1MusicCipher: React.FC<Stage1MusicCipherProps> = ({
  ...
  // 5-second Big Screen Song Reveal Banner
  ...
  const handleAward = (targetTeamId?: string) => {
    ...
    const teamObj = teams.find(t => t.id === teamToAward);
    ...
    // Trigger 5-second Big Screen Song Reveal Banner
    setRevealedBanner({
      isOpen: true,
      songTitle: activeSong.title,
      artistOrComposer: activeSong.artistOrComposer,
      category: activeSong.category === '80-ci illər' ? 'Retro mahnılar' : activeSong.category,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-xl rounded-3xl p-6 shadow-2xl relative overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="py-6 flex flex-col items-center text-center">
              ...
              {/* Music Player Button Controls (Play, Pause, Resume, Stop) */}
              <div className="flex items-center gap-3">
                {playbackStatus === 'idle' ? (
                  <button
                    id="stage1-play-btn"
                    onClick={handlePlayMusic}
                    className="px-6 py-3 rounded-2xl font-bold text-sm flex items-center gap-2.5 transition-all cursor-pointer shadow-lg bg-amber-500 hover:bg-amber-400 text-slate-950 shadow-amber-500/30 active:scale-95"
                  >
                  ...
                  <>
                    <button
                      ...
                      className="px-5 py-3 rounded-2xl font-bold text-sm flex items-center gap-2 transition-all cursor-pointer shadow-lg bg-amber-500 hover:bg-amber-400 text-slate-950 shadow-amber-500/30 active:scale-95"
                    >
                    ...
                    <button
                      ...
                      className="px-4 py-3 rounded-2xl font-bold text-sm flex items-center gap-2 transition-all cursor-pointer bg-slate-800 hover:bg-rose-900/60 text-rose-300 border border-slate-700 hover:border-rose-500/40 active:scale-95"
                    >
                  ...
                  <>
                    <button
                      id="stage1-resume-btn"
                      onClick={handleResumeMusic}
                      className="px-6 py-3 rounded-2xl font-bold text-sm flex items-center gap-2 transition-all cursor-pointer shadow-lg bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-600/30 active:scale-95"
                    >
                      <Play className="w-5 h-5 fill-current" />
                      <span>Davam Et (Pauzadan)</span>
                    </button>
                    <button
                      id="stage1-reset-btn"
                      onClick={handleStopMusic}
                      className="px-4 py-3 rounded-2xl font-bold text-sm flex items-center gap-2 transition-all cursor-pointer bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 active:scale-95"
                    >
              ...
              <p className="text-xs text-slate-400 mt-2">
                {playbackStatus === 'playing'
                  ? 'Mahnının melodiyası səslənir...'
                  : playbackStatus === 'paused'
                  ? 'Musiqi fasilədədir. Davam etmək üçün "Davam Et" düyməsinə basın.'
                  : 'Musiqini ifa etmək üçün düyməyə basın'}
              </p>
        ...
        >
          ...
          <div className="w-full max-w-2xl bg-slate-900/90 border border-slate-800 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-2xl">
            <div className="text-center sm:text-left">
              <span className="text-xs text-slate-400 block">Aparıcı üçün qərar:</span>
              <div className="flex items-center gap-2">
                <span className="text-sm font-bold text-slate-200">
                  {hostPeekSecret && activeSong
                    ? `Gizli baxış: "${activeSong.title}" (${activeSong.artistOrComposer})`
                    : 'Cavab düzgündürsə, komandaya xal verin'}
                </span>
                <button
                  type="button"
                  onClick={() => setHostPeekSecret(!hostPeekSecret)}
                  className="p-1 text-slate-400 hover:text-amber-400 text-xs rounded transition-colors"
                  title="Aparıcı üçün gizli baxış (yalnız aparıcı görsün)"
                >
                  {hostPeekSecret ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                </button>
              </div>
            </div>
      ...
      {/* 5-Second Big Screen Song Reveal Banner on Correct Answer */}
      {revealedBanner && (
        <SongRevealedBanner
