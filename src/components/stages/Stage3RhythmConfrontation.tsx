...
const POINT_BRACKETS = [
  ...
  { label: '50+ saniyə', min: 50, max: 9999, points: 0, color: 'text-slate-400 border-slate-700 bg-slate-800/40' },
];
...
export const Stage3RhythmConfrontation: React.FC<Stage3Props> = ({
  ...
  // 5-second Big Screen Song Reveal Banner state
  ...
  const handleBuzz = useCallback((teamId: string) => {
    ...
    // Immediately pause timer and audio
  ...
  // Stopwatch effect (No 60s hard stop on audio, continues naturally)
  useEffect(() => {
    if (isTimerRunning) {
  ...
  // Award Points and Show 5-second Reveal Banner
  const handleAward = (targetTeamId?: string) => {
    ...
    const winningTeam = teams.find(t => t.id === teamToAward);

    // Show 5-second Big Screen Reveal Banner
    setRevealedBanner({
      isOpen: true,
      songTitle: currentSong.title,
      artistOrComposer: currentSong.artistOrComposer,
      category: `Mahnı ${currentSongIndex + 1} • Ritm Qarşıdurması`,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900/90 border border-slate-800 rounded-3xl p-6 flex flex-col justify-between shadow-xl">
          ...
          <div className="py-8 flex flex-col items-center text-center">
            ...
            {/* Comprehensive Audio & Stopwatch Controls (Play / Pause / Resume / Stop / Reset) */}
            <div className="flex flex-wrap items-center justify-center gap-3">
              {playbackStatus === 'idle' ? (
                <button
                  id="stage3-start-timer-btn"
                  onClick={handleStartPlayAndTimer}
                  className="px-6 py-3 rounded-2xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-sm flex items-center gap-2 shadow-lg shadow-amber-500/30 transition-all cursor-pointer"
                >
                ...
                <>
                  <button
                    id="stage3-pause-timer-btn"
                    onClick={handlePause}
                    className="px-5 py-3 rounded-2xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-sm flex items-center gap-2 shadow-lg shadow-amber-500/20 cursor-pointer"
                  >
                    <Pause className="w-4 h-4 fill-current" />
                    <span>Fasilə (Pauza)</span>
                  </button>

                  <button
                    id="stage3-stop-timer-btn"
                    onClick={handleStopTimer}
                    className="px-5 py-3 rounded-2xl bg-rose-600 hover:bg-rose-500 text-white font-bold text-sm flex items-center gap-2 shadow-lg shadow-rose-900/50 cursor-pointer"
                  >
                    <Square className="w-4 h-4 fill-current" />
                    <span>Dayandır (Xalı Fikslə)</span>
                  </button>
                </>
              ) : (
                <>
                  <button
                    id="stage3-resume-timer-btn"
                    onClick={handleResume}
                    className="px-5 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm flex items-center gap-2 shadow-lg shadow-emerald-950 cursor-pointer"
                  >
                    <Play className="w-4 h-4 fill-current" />
                    <span>Davam Et (İfa)</span>
                  </button>

                  <button
                    id="stage3-stop-paused-btn"
                    onClick={handleStopTimer}
                    className="px-5 py-3 rounded-2xl bg-rose-600 hover:bg-rose-500 text-white font-bold text-sm flex items-center gap-2 shadow-lg shadow-rose-900/50 cursor-pointer"
                  >
                    <Square className="w-4 h-4 fill-current" />
              ...
              <button
                ...
                className="p-3 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition-colors cursor-pointer flex items-center gap-1.5 text-xs font-semibold"
              >
          ...
          <div className="pt-4 border-t border-slate-800 space-y-4">
            {!isRevealed ? (
              <div className="flex flex-wrap items-center justify-between gap-2 p-2.5 rounded-xl bg-slate-950/40 border border-slate-800">
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-400">Mahnı adı gizlidir (Ekrana çıxmır)</span>
                  {showHostSecretPeek && (
                    <span className="text-xs px-2 py-0.5 rounded bg-amber-500/20 text-amber-300 font-mono font-bold border border-amber-500/30">
                      {currentSong?.title} &bull; {currentSong?.artistOrComposer}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    <span>{showHostSecretPeek ? 'Gizlə' : 'Aparıcı Baxışı'}</span>
                  </button>
                  <button
            ...
            <div className="text-center sm:text-left">
              ...
              <span className="text-sm font-bold text-slate-200">
                {isRevealed && currentSong ? (
                  `"${currentSong.title}" üçün xal verilsin?`
                ) : (
                  `Mahnı ${currentSongIndex + 1} — Cavab düzgündürmü?`
                )}
              </span>
            </div>
